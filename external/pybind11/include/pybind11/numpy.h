...
/* This will be true on all flat address space platforms and allows us to reduce the
   whole npy_intp / Py_ssize_t  / Py_intptr_t business down to just Py_ssize_t  for all size
   and dimension types (e.g. shape, strides, indexing), instead of inflicting this
   upon the library user. */
static_assert(sizeof(::pybind11::Py_ssize_t ) == sizeof(Py_intptr_t), "Py_ssize_t  != Py_intptr_t");
static_assert(std::is_signed<Py_intptr_t>::value, "Py_intptr_t must be signed");
// We now can reinterpret_cast between py::Py_ssize_t  and Py_intptr_t (MSVC + PyPy cares)
...
template <Py_ssize_t  Dim = 0, typename Strides>
Py_ssize_t  byte_offset_unsafe(const Strides &) {
    return 0;
}
template <Py_ssize_t  Dim = 0, typename Strides, typename... Ix>
Py_ssize_t  byte_offset_unsafe(const Strides &strides, Py_ssize_t  i, Ix... index) {
 ...
 */
template <typename T, Py_ssize_t  Dims>
...
protected:
    ...
    conditional_t<Dynamic, const Py_ssize_t  *, std::array<Py_ssize_t , (size_t) Dims>> shape_, strides_;
    const Py_ssize_t  dims_;
    ...
    unchecked_reference(const void *data,
                        const Py_ssize_t  *shape,
                        const Py_ssize_t  *strides,
                        enable_if_t<Dyn, Py_ssize_t > dims)
...
public:
    ...
    const T &operator()(Ix... index) const {
        static_assert(Py_ssize_t {sizeof...(Ix)} == Dims || Dynamic,
                      "Invalid number of indices for unchecked array reference");
        return *reinterpret_cast<const T *>(data_
                                            + byte_offset_unsafe(strides_, Py_ssize_t (index)...));
    }
    /**
     * Unchecked const reference access to data; this operator only participates if the reference
     * is to a 1-dimensional array.  When present, this is exactly equivalent to `obj(index)`.
     */
    template <Py_ssize_t  D = Dims, typename = enable_if_t<D == 1 || Dynamic>>
    const T &operator[](Py_ssize_t  index) const {
    ...
    const T *data(Ix... ix) const {
        return &operator()(Py_ssize_t (ix)...);
    }

    /// Returns the item size, i.e. sizeof(T)
    constexpr static Py_ssize_t  itemsize() { return sizeof(T); }

    /// Returns the shape (i.e. size) of dimension `dim`
    Py_ssize_t  shape(Py_ssize_t  dim) const { return shape_[(size_t) dim]; }

    /// Returns the number of dimensions of the array
    Py_ssize_t  ndim() const { return dims_; }

    /// Returns the total number of elements in the referenced array, i.e. the product of the
    /// shapes
    template <bool Dyn = Dynamic>
    enable_if_t<!Dyn, Py_ssize_t > size() const {
        return std::accumulate(
            shape_.begin(), shape_.end(), (Py_ssize_t ) 1, std::multiplies<Py_ssize_t >());
    }
    template <bool Dyn = Dynamic>
    enable_if_t<Dyn, Py_ssize_t > size() const {
        return std::accumulate(shape_, shape_ + ndim(), (Py_ssize_t ) 1, std::multiplies<Py_ssize_t >());
    }
    ...
    /// slice).
    Py_ssize_t  nbytes() const { return size() * itemsize(); }
};
...
template <typename T, Py_ssize_t  Dim>
...
public:
    ...
    dtype(list names, list formats, list offsets, Py_ssize_t  itemsize) {
        dict args;
    ...
    Py_ssize_t  itemsize() const { return detail::array_descriptor_proxy(m_ptr)->elsize; }
...
private:
    ...
    dtype strip_padding(Py_ssize_t  itemsize) {
...
public:
    ...
    using ShapeContainer = detail::any_container<Py_ssize_t >;
    using StridesContainer = detail::any_container<Py_ssize_t >;
    ...
    template <typename T>
    explicit array(Py_ssize_t  count, const T *ptr, handle base = handle())
    ...
    Py_ssize_t  size() const {
        return std::accumulate(shape(), shape() + ndim(), (Py_ssize_t ) 1, std::multiplies<Py_ssize_t >());
    }
    ...
    Py_ssize_t  ndim() const { return detail::array_proxy(m_ptr)->nd; }

    /// Base object
    object base() const { return reinterpret_borrow<object>(detail::array_proxy(m_ptr)->base); }

    /// Dimensions of the array
    const Py_ssize_t  *shape() const { return detail::array_proxy(m_ptr)->dimensions; }

    /// Dimension along a given axis
    Py_ssize_t  shape(Py_ssize_t  dim) const {
        if (dim >= ndim()) {
    ...
    const Py_ssize_t  *strides() const { return detail::array_proxy(m_ptr)->strides; }

    /// Stride along a given axis
    Py_ssize_t  strides(Py_ssize_t  dim) const {
        if (dim >= ndim()) {
            fail_dim_check(dim, "invalid axis");
        }
        return strides()[dim];
    ...
    template <typename... Ix>
    Py_ssize_t  offset_at(Ix... index) const {
        if ((Py_ssize_t ) sizeof...(index) > ndim()) {
            fail_dim_check(sizeof...(index), "too many indices for an array");
        }
        return byte_offset(Py_ssize_t (index)...);
    }

    Py_ssize_t  offset_at() const { return 0; }

    /// Item count from beginning of the array to a given index (full or partial).
    /// May throw if the index would lead to out of bounds access.
    template <typename... Ix>
    Py_ssize_t  index_at(Ix... index) const {
     ...
     */
    template <typename T, Py_ssize_t  Dims = -1>
...
protected:
    ...
    void fail_dim_check(Py_ssize_t  dim, const std::string &msg) const {
        throw index_error(msg + ": " + std::to_string(dim) + " (ndim = " + std::to_string(ndim())
                          + ')');
    }

    template <typename... Ix>
    Py_ssize_t  byte_offset(Ix... index) const {
        check_dimensions(index...);
        return detail::byte_offset_unsafe(strides(), Py_ssize_t (index)...);
    }
    ...
    void check_dimensions(Ix... index) const {
        check_dimensions_impl(Py_ssize_t (0), shape(), Py_ssize_t (index)...);
    }

    void check_dimensions_impl(Py_ssize_t , const Py_ssize_t  *) const {}

    template <typename... Ix>
    void check_dimensions_impl(Py_ssize_t  axis, const Py_ssize_t  *shape, Py_ssize_t  i, Ix... index) const {
        if (i >= *shape) {
...
public:
    ...
    explicit array_t(Py_ssize_t  count, const T *ptr = nullptr, handle base = handle())
        : array({count}, {}, ptr, base) {}

    constexpr Py_ssize_t  itemsize() const { return sizeof(T); }
    ...
    const T &at(Ix... index) const {
        ...
        return *(static_cast<const T *>(array::data())
                 + byte_offset(Py_ssize_t (index)...) / itemsize());
    }
...
public:
    using container_type = std::vector<Py_Py_ssize_t  >;
...
public:
    using container_type = std::vector<Py_ssize_t >;
...
template <size_t N>
broadcast_trivial
broadcast(const std::array<buffer_info, N> &buffers, Py_ssize_t  &ndim, std::vector<Py_ssize_t > &shape) {
    ndim = std::accumulate(
        buffers.begin(), buffers.end(), Py_ssize_t (0), [](Py_ssize_t  res, const buffer_info &buf) {
    ...
    for (size_t i = 0; i < N && (trivial_broadcast_c || trivial_broadcast_f); ++i) {
        ...
        if (trivial_broadcast_c) {
            Py_ssize_t  expect_stride = buffers[i].itemsize;
...
struct vectorize_returned_array {
    ...
    static Type create(broadcast_trivial trivial, const std::vector<Py_ssize_t > &shape) {
...
struct vectorize_returned_array<Func, void, Args...> {
    ...
    static Type create(broadcast_trivial, const std::vector<Py_ssize_t > &) { return none(); }
...
private:
    ...
    void apply_broadcast(std::array<buffer_info, NVectorized> &buffers,
                         ...
                         Return *out,
                         size_t size,
                         const std::vector<Py_ssize_t > &output_shape,
                         index_sequence<Index...>,
                         index_sequence<VIndex...>,
                         index_sequence<BIndex...>) {
